...
class PPOAgent:
    ...
    def __init__(self, env, lr=3e-4, gamma=0.999, gae_lambda=0.95, value_coeff=0.5,
        ...
        # Adaptive entropy parameters using a direct exponential decay function
        self.ema_alpha = ema_alpha
        self.entropy_coeff = 0.01  # Initial entropy coefficient
        # Constants for the exponential decay function: A * exp(-B * ema_reward)
        self.exp_decay_A = 0.01
        self.exp_decay_B = 0.01151
    ...
    def update2(self, rollouts):
        ...
        if self.ema_reward is not None:
            # Use a direct exponential decay function.
            # Clip the ema_reward to 0 to prevent the entropy from increasing with negative rewards.
            reward_for_decay = max(0, self.ema_reward)
            new_coeff = self.exp_decay_A * np.exp(-self.exp_decay_B * reward_for_decay)

            # Clip the value to ensure it stays within the desired range [0.001, 0.01]
            self.entropy_coeff = np.clip(new_coeff, 0.001, 0.01)
