...
class PPOAgent:
    ...
    def __init__(self, env, lr=3e-4, gamma=0.999, gae_lambda=0.95, value_coeff=0.5,
                 ...
                 # Adaptive entropy parameters for update2
                 initial_entropy_coeff=0.005,
                 min_entropy_coeff=0.001,
                 max_entropy_coeff=0.05,
                 target_entropy_reward=180, # Adjusted to a more achievable goal
                 ema_alpha=0.1,
                 entropy_power=1.0): # Parameter to control decay shape
        """
        ...
        self.ent_coef = ent_coef # Fixed entropy coefficient
        ...
        self.target_entropy_reward = target_entropy_reward
        self.ema_alpha = ema_alpha
        self.entropy_coeff = initial_entropy_coeff
        self.entropy_power = entropy_power
    ...
    def update(self, rollouts):
        """
        Performs a PPO update step with a fixed entropy coefficient.
        """
        # Unpack rollouts
        ...
        for _ in range(self.n_epochs):
            ...
            for start in range(0, self.n_steps, self.minibatch_size):
                ...
                # Entropy Loss
                entropy_loss = -self.ent_coef * current_entropies.mean()
    ...
    def update2(self, rollouts):
        ...
        if self.ema_reward is not None:
            # Normalize EMA reward to a 0-1 scale
            normalized_reward_01 = (self.ema_reward - (-self.target_entropy_reward)) / (2 * self.target_entropy_reward)
            normalized_reward_01 = np.clip(normalized_reward_01, 0, 1)

            # Use a power function to create the non-linear "exponential" decay
            # A higher entropy_power leads to a faster decay as performance improves
            entropy_map = (1 - normalized_reward_01) ** self.entropy_power
            target_entropy = self.min_entropy_coeff + (self.max_entropy_coeff - self.min_entropy_coeff) * entropy_map

            # Directly set the entropy coefficient to the new target
            self.entropy_coeff = np.clip(target_entropy, self.min_entropy_coeff, self.max_entropy_coeff)
...
if __name__ == "__main__":
    ...
    agent = PPOAgent(env, entropy_power=3.0)
