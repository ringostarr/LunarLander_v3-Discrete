...
class PPOAgent:
    ...
    def update2(self, rollouts):
        ...
        if self.ema_reward is not None:
            normalized_ema = np.clip(self.ema_reward / self.target_entropy_reward, -1, 1)
            normalized_ema = (normalized_ema+1)/2
            target_entropy = self.min_entropy_coeff + (self.max_entropy_coeff - self.min_entropy_coeff) * (1 - normalized_ema)
